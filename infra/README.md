...
| `libvirt-host-image/` | bootc `Containerfile` (+ build/boot instructions) for a disposable Fedora VM that the `libvirt` provider type adopts as a host — for running Troshka fully locally | `podman build`/`bootc-image-builder`/`virt-install` (see [libvirt-host-image/README.md](libvirt-host-image/README.md)) |
