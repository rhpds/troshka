...
def create_project_from_template(
    ...
    if bastion_image_id:
        ...
        if item:
            bastion_image = {"id": item.id, "name": item.name, "size_gb": max(1, (item.size_bytes or 0) // (1024 ** 3))}
    ...
    if bastion_iso_id:
        ...
        if iso_item:
            bastion_iso = {"id": iso_item.id, "name": iso_item.name, "size_bytes": iso_item.size_bytes or 0}
