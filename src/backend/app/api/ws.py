...
async def console_websocket_proxy(websocket: WebSocket, token: str):
    ...
    Validates one-time token, then proxies binary frames to local websockify."""
    import websockets

    from app.services.console_proxy import consume_console_token

    info = consume_console_token(token)
    if not info:
    ...
    try:
        ...
        await websocket.close(code=4002, reason="Console proxy not available")
        return
    ...
    try:
        await asyncio.gather(_client_to_backend(), _backend_to_client())
    except Exception:
        pass
    finally:
        await backend.close()
