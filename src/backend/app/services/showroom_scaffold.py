...
# Short, DNS-safe codes for app-proxy route names. Setting an explicit route
# spec.host needs the routes/custom-host permission, which the restricted RHDP
# sandbox denies — so we use short route names and let OpenShift auto-generate the
...
def app_proxy_route_name(project_id: str, internal_host: str) -> str:
    """OCP Route metadata.name for an app-proxy host: tpf-<pid8>-<code>. Kept short
    so the auto-generated host (<name>-<namespace>.<apps-domain>) stays within the
    63-char DNS label limit without needing routes/custom-host."""
    return f"tpf-{project_id[:8]}-{app_proxy_route_code(internal_host)}"


def app_proxy_public_host(
    project_id: str, internal_host: str, apps_domain: str, namespace: str
) -> str:
    """Deterministic public hostname OpenShift auto-generates for an app-proxy
    route: <route-name>-<namespace>.<apps-domain>."""
    return (
        f"{app_proxy_route_name(project_id, internal_host)}-{namespace}.{apps_domain}"
    )
...
def build_app_proxy_config(
    ...
    # The public host is auto-generated by OpenShift as
    #   tpf-<pid8>-<code>-<namespace>.apps.<cluster>
    # The server_name regex captures <pid8> and everything after the code
    # (troshka_ns = "<namespace>.apps.<cluster>"), so config stays project- and
    # cluster-agnostic; only the per-host <code> is baked. Any internal host's
    # public equivalent is reconstructed as tpf-$troshka_pid-<code>-$troshka_ns.
    for h in internal_hosts:
        ...
        sub_filters.append(
            f'    sub_filter "//{h}" '
            f'"//tpf-$troshka_pid-{app_proxy_route_code(h)}-$troshka_ns";'
        )
    # Rewrite redirect Location host -> public. label->code is not generic, so emit
    # one redirect per internal host (redirect_uri query params keep .local so the
    # OAuthClient still validates).
    redirects: list[str] = []
    for h in internal_hosts:
        # First label has no dots (regex-safe as-is); only the apps domain needs
        # its dots escaped for the nginx regex.
        label = h.split(".")[0]
        host_apps_re = re.escape(derive_apps_domain(h))
        redirects.append(
            f"    proxy_redirect ~^https://{label}\\.{host_apps_re}"
            f"(?<troshka_rest>.*)$ "
            f"https://tpf-$troshka_pid-{app_proxy_route_code(h)}-$troshka_ns"
            "$troshka_rest;"
        )
    ...
    return [
        ...
        },
        {
            ...
            "command": (
                ...
                # 1001) under podman on troshkad hosts. Hand that uid ownership of
                # www so antora can create its output tree (modules/, _/); otherwise
                # it gets EACCES on mkdir /showroom/www/modules. chown (not a
                # world-writable 0777) keeps www writable only by the builder and
                # root; it works because root creates the dir and both containers
                # share this volume and the pod's user namespace. No-op on the k8s
                # path where runAsUser:0 makes antora root and already owns the tree.
                "chown 1001 /showroom/www && "
