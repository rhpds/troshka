...
def generate_topology_from_template(resolved: dict) -> dict:
    ...
    edges.append({
        "id": f"e-{uuid.uuid4()}", "source": bastion_id, "target": cluster_net_id,
    ...
    for i in range(control_count):
        ...
        dp_id = f"dp-{uuid.uuid4()}"
        nodes.append({
            "id": vm_id, "type": "vmNode",
            "position": {"x": bastion_x_offset + i * x_spacing, "y": cp_y},
            "data": {
                "name": f"cp-{i}", "label": f"cp-{i}",
                "vcpus": resolved.get("control_vcpus", 4),
                "ram": resolved.get("control_ram_gb", 16),
                "os": "rhcos", "icon": "\U0001F5A5", "firmware": "uefi",
                "powerOnAtDeploy": True, "bootMethod": "disk", "bmcEnabled": True,
                "nics": [
    ...
    for i in range(worker_count):
        ...
        nodes.append({
            ...
            "data": {
                "name": f"worker-{i}", "label": f"worker-{i}",
