...
class TestPollHosts(unittest.TestCase):
    ...
    @patch("app.core.database.SessionLocal")
    @patch("app.services.troshkad_client.check_health")
    def test_failed_health_disconnects_after_timeout(
        self, mock_check, mock_session_cls
    ):
        from app.services.health_poller import _poll_hosts
        ...
        host.id = "test-host-uuid-1234"
        host.agent_status = "connected"
        host.last_health_at = datetime.now(UTC) - timedelta(seconds=120)
        host.agent_token = "token123"
        ...
        mock_check.return_value = {
            ...
            "partitions": [
                ...
                },
                {
                    ...
                    "used_bytes": 225 * 1024**3,
                    "free_bytes": 275 * 1024**3,
                    "device": "/dev/nvme1n1",
                    "fstype": "xfs",
                },
            ],
    ...
    @patch("app.core.database.SessionLocal")
    @patch("app.services.troshkad_client.check_health")
    def test_partition_warnings_cleared_when_healthy(
        self, mock_check, mock_session_cls
    ):
        from app.services.health_poller import _poll_hosts
