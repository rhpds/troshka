...
def test_full_agnosticd_flow():
    ...
    lookup_resp = client.get(
        "/api/v1/patterns", params={"name": "Integration Test Pattern"}, headers=HEADERS
    )
    ...
    bastion = [
        n
        for n in project_topo["nodes"]
        if n["type"] == "vmNode"
        and "bastions" in n["data"].get("tags", {}).get("AnsibleGroup", "")
    ][0]
    ...
    controllers = [
        ...
        for n in project_topo["nodes"]
        if n["type"] == "vmNode"
        and "controllers" in n["data"].get("tags", {}).get("AnsibleGroup", "")
    ]
