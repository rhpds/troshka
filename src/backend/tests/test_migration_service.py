...
def test_validate_migration_same_pool():
    ...
    source = Host(provider_id=provider.id, state="active", agent_status="connected",
                  storage_pool_id=pool.id, total_vcpus=64, total_ram_mb=262144)
    target = Host(provider_id=provider.id, state="active", agent_status="connected",
                  storage_pool_id=pool.id, total_vcpus=64, total_ram_mb=262144)
...
def test_validate_migration_different_pools():
    ...
    source = Host(provider_id=provider.id, state="active", agent_status="connected",
                  storage_pool_id=pool_a.id, total_vcpus=64, total_ram_mb=262144)
    target = Host(provider_id=provider.id, state="active", agent_status="connected",
                  storage_pool_id=pool_b.id, total_vcpus=64, total_ram_mb=262144)
    db.add_all([source, target])
    db.flush()
