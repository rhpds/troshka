...
class TestBuildMonRestoreDatavolume:
    def test_uses_fixed_mon_name_and_archive_content_type(self):
        dv = build_mon_restore_datavolume(
            "troshka-abc", _MON_DEVICE, _S3_CONFIG, _DEFAULT_SECRET
        )
        assert dv["metadata"]["name"] == CEPH_MON_PVC_NAME
        assert dv["metadata"]["namespace"] == "troshka-abc"
        assert dv["spec"]["contentType"] == "archive"
        assert dv["spec"]["source"]["s3"]["secretRef"] == _DEFAULT_SECRET
        assert "ceph-mon-0.tar.gz" in dv["spec"]["source"]["s3"]["url"]
        # No explicit volumeMode -> defaults to Filesystem (required for archive untar).
        assert "volumeMode" not in dv["spec"]["pvc"]
...
class TestBuildCephRestoreDatavolumes:
    def test_none_when_no_restore_capture(self):
        assert build_ceph_restore_datavolumes("ns", None, _S3_CONFIG) == (None, [], "", [])

    def test_none_when_empty_devices(self):
        result = build_ceph_restore_datavolumes("ns", {"mon": None, "osds": []}, _S3_CONFIG)
        assert result == (None, [], "", [])

    def test_full_capture_block_emits_correct_pvc_dv_list(self):
        ...
        mon_dv, osd_dvs, mon_name, osd_names = build_ceph_restore_datavolumes(
            "troshka-abc", restore_capture, _S3_CONFIG, {"bucket": "central"}, "ocs-storage"
        )
        assert mon_dv is not None
        assert mon_name == CEPH_MON_PVC_NAME
        # OSDs sorted by index regardless of input order.
        assert osd_names == ["osd-restore-data-0", "osd-restore-data-1"]
        assert [dv["metadata"]["name"] for dv in osd_dvs] == osd_names
        # OSD 0 is "obc"-sourced -> falls back to project s3Config (no obcConfig here).
        assert osd_dvs[0]["spec"]["source"]["s3"]["secretRef"] == _DEFAULT_SECRET
        # OSD 1 is "central"-sourced -> uses central_s3_config's secret.
...
class TestMaterializeCephRestorePvcs:
    def test_creates_mon_and_osd_datavolumes(self):
        custom_api = MagicMock()
        restore_capture = {"mon": _MON_DEVICE, "osds": [_OSD_DEVICE_0]}

        mon_name, osd_names = materialize_ceph_restore_pvcs(
            custom_api, "troshka-abc", restore_capture, _S3_CONFIG, storage_class="ocs-storage"
        )
        ...
        created_names = {
            ...
            for call in custom_api.create_namespaced_custom_object.call_args_list
        }
        assert created_names == {CEPH_MON_PVC_NAME, "osd-restore-data-0"}

    def test_returns_empty_when_no_capture(self):
        custom_api = MagicMock()
        mon_name, osd_names = materialize_ceph_restore_pvcs(custom_api, "ns", None, _S3_CONFIG)
    ...
    def test_idempotent_on_409(self):
        ...
        mon_name, osd_names = materialize_ceph_restore_pvcs(
            custom_api, "ns", restore_capture, _S3_CONFIG
        )
        assert mon_name == CEPH_MON_PVC_NAME
        assert osd_names == []
    ...
    def test_reraises_non_409_api_exception(self):
        ...
        with pytest.raises(ApiException):
            materialize_ceph_restore_pvcs(custom_api, "ns", restore_capture, _S3_CONFIG)
...
class TestWaitForCephRestoreDatavolumes:
    ...
    def test_returns_once_all_succeeded(self):
        custom_api = MagicMock()
        custom_api.get_namespaced_custom_object.return_value = {"status": {"phase": "Succeeded"}}
        asyncio.run(
            wait_for_ceph_restore_datavolumes(
                custom_api, "ns", ["rook-ceph-mon-a", "osd-restore-data-0"]
            )
        )
    ...
    def test_raises_on_failed_phase(self):
        custom_api = MagicMock()
        custom_api.get_namespaced_custom_object.return_value = {"status": {"phase": "Failed"}}
        with pytest.raises(RuntimeError, match="failed to import"):
            asyncio.run(
                wait_for_ceph_restore_datavolumes(custom_api, "ns", ["osd-restore-data-0"])
            )
    ...
    def test_polls_until_succeeded(self):
        ...
        asyncio.run(
            wait_for_ceph_restore_datavolumes(
                custom_api, "ns", ["osd-restore-data-0"], max_wait_seconds=10, sleep_seconds=1
            )
        )
        assert custom_api.get_namespaced_custom_object.call_count == 2
